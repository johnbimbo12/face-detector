...
img = cv2.imread('john11.jpg')        # -------------->>>>>>>>>>>>>>>>>>  The Image to be checked
...
for (x, y, w, h) in faces:
    ...
    nei_conf = []
    for _ in range(13):
        recog = cv2.face.LBPHFaceRecognizer_create(int(radPixel), neighbour)  # creating FACE RECOGNISER
    ...
    plt.subplot(2, 1, 1)
    plt.plot(nei_ID)
    plt.title('ID vs number of neighbours', fontsize=10)
    plt.axis([0, neighbour, 10, 25])
    plt.ylabel('ID', fontsize=8)
    plt.xlabel('Number of neighbours', fontsize=8)
    p2 = plt.subplot(2, 1, 2)
    plt.plot(nei_conf, 'red')
    plt.title('Confidence vs number of neighbours', fontsize=10)
    p2.set_xlim(xmin=0)
    ...
    cell_conf = []
    for _ in range(50):
        recog = cv2.face.LBPHFaceRecognizer_create(int(radPixel), int(neighbour), cellVal, cellVal)  # creating FACE RECOGNISER
    ...
    plt.subplot(3, 2, 5)
    plt.plot(cell_ID)
    plt.title('ID vs number of cells', fontsize=10)
    plt.axis([0, 49, 0, 25])
    plt.ylabel('ID', fontsize=8)
    plt.xlabel('Number of Cells', fontsize=8)
    plt.subplot(3, 2, 6)
    plt.plot(cell_conf, 'red')
    plt.title('Confidence vs number of cells', fontsize=10)
    plt.ylabel('Confidence', fontsize=8)
    plt.xlabel('Number of Cells', fontsize=8)
    plt.tight_layout()
    plt.show()
