...
#LBPH = cv2.face.createLBPHFaceRecognizer(2, 2, 7, 7, 20)
...
img = cv2.imread('john11.jpg')                  # ------->>> THE ADDRESS TO THE PHOTO
#img = cv2.imread('funbi.png')                  # ------->>> THE ADDRESS TO THE PHOTO
...
for (x, y, w, h) in faces:                                  # Frames  LOCATION X, Y  WIDTH, HEIGHT
    ...
    NameFind.DispID(x, y, w, h, NAME + " (LBPH)", gray)

    ID, conf = EIGEN.predict(Face)                          # EIGEN FACE RECOGNITION
    NAME = NameFind.ID2Name(ID, conf)
    NameFind.DispID3(x, y, w, h, NAME + " (Eigenface)", gray)

    ID, conf = FISHER.predict(Face)                         # FISHER FACE RECOGNITION
    NAME = NameFind.ID2Name(ID, conf)
    NameFind.DispID2(x, y, w, h, NAME + " (Fisherface)", gray)

cv2.imshow('All in one recognition system', gray)           # IMAGE DISPLAY
cv2.waitKey(0)
